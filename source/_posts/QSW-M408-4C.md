...
個人向けとして関係しそうな内容に絞って記載します。また、NAS（ネットワークストレージサーバー）向けに10GbE SFP+ポートも4つ持っていますが、私はSFP＋ポートは使っておらず通常のイーサネットポート（RJ45)を使った説明になりますのでご了承ください。
...
過去3分間の履歴と記載があります。なぜ3分固定なんでしょうか{% emoji unamused %}。また、この画面を開いてからの履歴が画面上で記録されていくだけであり、スイッチ本体に記録されているものではなさそうです。
...
私はVLANを利用していませんが、試しにPort3〜Port6までひとつのVLANを切り、VLANIDに100と振ってみました。VLANを設定すると上記画面のように設定した内容が表示されます。ネットワークが好きな方はセキュリティ目的別にVLANを組むでしょうし、またはスピード重視でジャンボフレームの設定有無（1500/9K）でVLANを切り、バックアップ用途のネットワークを9Kに設定される使い方が多いでしょうか。
...
1. Port9に向かう送信元のMACアドレス、つまりサーバーにアクセスを許可するクライアントのMACアドレスを全て列挙しています。
2. 宛先は指定なしとなっていますが、Port9には当該サーバー1台しか接続されていない想定であり、特に指定をしていません。
3. サーバーからインターネットへのアクセスを許可する場合は、インターネットに接続するルーターのLAN側MACアドレスも登録します。
4. 最後の行で該当しないMACアドレスからの送信は拒否しています。
...
スイッチであっても10Gbpsのチップはまだ発熱量が大きいのが現状です。もし、このスイッチをお使いになる場合は、筐体下面についている短いゴム足をもう少し長いものに変えたり、縦置きにするなど少し工夫が必要と思います。それでもこの製品はそれなりの小型化と静粛さと高速化を両立したバランスの良いスイッチであると思えます。あとはネットワーク統計やエラーパケットなどを把握するための機能搭載を期待するところです。

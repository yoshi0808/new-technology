...
左ペインのルールとポリシーからルールとポリシーを選択します。以前の記事"{% post_link rule-policy2 %}"において、デフォルトのルール"To_Internet"を作成しました。{% label primary@Traffic to WAN %}のグループ配下に"To_Internet"ルールがある事を確認してください。このファイアウォールルールはLANおよびVPNからWANへの接続を許可するシンプルなものですが、今回これは修正しません。XG v18では、SSLインスペクションとファイアウォールルールは互いに独立していて、別メニューで設定する事になります。

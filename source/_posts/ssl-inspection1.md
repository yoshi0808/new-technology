...
ここは個人の「決め」ですが、上記で証明書がインストール出来るグループを"IPホスト"としてXGに登録しましょう。左ペインの{% label @ホストとサービス %}から、IPホストのタブをクリックし追加ボタンをクリックします。以下の通り、ホストのIPの範囲を示す名前を入力し、IPの範囲を入力して保存ボタンをクリックします。ここでは、{% label @CA_Client_IPv4 %}として登録する事にします。

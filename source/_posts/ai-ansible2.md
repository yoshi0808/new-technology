...
なお、Codexの場合は、これまでCodex-CLIを使っていないクリーンな環境でCodex拡張機能を使う場合は、ホームディレクトリに.codexのフォルダを作る必要があるようです（VSCodeの表示→出力でCodexを選択すると、以下のエラーメッセージが確認できます）。

``` text
2026-07-18 06:23:06.547 [error] [CodexMcpConnection] cli: message="WARNING: proceeding, even though we could not create PATH aliases: CODEX_HOME points to \"/home/yoshi/.codex\", but that path does not exist\nError: CODEX_HOME points to \"/home/yoshi/.codex\", but that path does not exist"
```
よって、以下のコマンドを投入し、VS Codeを再起動してください。

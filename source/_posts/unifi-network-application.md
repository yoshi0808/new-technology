...
ここまででUniFiのセットアップは完了です。ESXiのスナップショットを取得していらした方はスナップショットの管理から「すべて削除」し、一旦、Ubuntu全体のバックアップを取得される事をお勧めします。今後、UniFi Network Applicationの運用全般（設定のバックアップ、ログ、通知、機器へのSSH）、スイッチの設定の紹介、Wi-Fiアクセスポイント設定の具体例を順次説明していきます。

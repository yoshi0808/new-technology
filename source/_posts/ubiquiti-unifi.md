...
なお、セキュリティゲートウェイの機能という意味ではUDMやUDM ProはIDS/IPS、DPI（ディープパケットインスペクション）の機能を持ち、DNSフィルタやパケットのシグネチャを検査する仕組みを持っています。DPIは通信するPort番号に関わらず、TCP通信のデータの中身を見てそれが何のアクセスデータか分類したり脅威分析を行いますが、暗号化されたSSL/TSL通信を解読することはできません。今日ではAWSはもちろんのこと、Akamai、CloudflareなどのCDN（コンテンツデリバリネットワーク）が台頭してきており、接続先のサーバー名やドメイン名だけでは容易にアプリケーションの正当性の判断ができなくなってきています。SSL/TLSの解読にはXG FirewallなどのSSL/TLSインスペクションの機能を持つ製品が必要です。
VPNはPPTP、L2TPに加え、最近TeleportVPNに対応しています。
...
特にスマホアプリから行うWi-Fi機器やスイッチ機器のモニタリング、設定変更が容易で秀逸です。コントローラは2要素認証に対応しており、自宅でコントローラにログインすると、スマホにPush通知が来ますのでこれに応答することによりログインが完了します。またアプリからはLANの直接接続、或いはUbiquitiのクラウド経由で自宅のUniFi Consoleと接続されますが、クラウド経由の場合でも自宅のFirewallやルーターで特定のPortで待ち受ける（Port解放する）ことなく、UniFi Console側からUbiquitiのクラウドに接続しに行く形態なので、セキュリティ的にも一段レベルが上で、ダイナミックDNSなどの準備が不要であることが特徴です。
...
<video src='ar.mov' type='video/mp4' controls='controls'  width='50%' height='50%'>
</video>
...
- MMモジュール（10Gbps マルチモード光学モジュール） 2つで4,832円
- 10GSFP+ RJ45モジュール 6,995円
- 10GのDAC1m 1,904円

日本のサイトでは今のところ送料が無料ですし全体的にお得だと思います。UbiquitiのMMモジュールは様々な機器に繋いでいますが、特段問題が発生したことはありません。

ケーブルに関してはUbiquitiの純正のOM3ケーブルは部屋と部屋とを繋ぐにしては短すぎるものばかりなので別途FSやAmazonなどで調達する必要があります。Amazon BasicのOM3ケーブルは20mで922円と廉価です。
...
UniFiスイッチはL2の製品群が主流です。設定はGUIのみで行います。SSHでスイッチに接続し、設定変更は行えますが再起動のタイミングにおいて、GUIで設定した内容に上書きされます。見える化やスイッチ全体の一括管理のために、全てのスイッチをUniFiのものに統一したくなりますが、部分的にUniFiスイッチを導入しても問題はありませんUniFiスイッチはハードウェア自体は一般的なL2スイッチと何ら違いは無いので異なるメーカーのL2スイッチをUniFiスイッチに接続できます。

個人で利用する場合、無理にVLANを切る必要もありませんが、マルチプルVLANも対応しているので、一つのネットワークアドレスで複数のVLANに属する形を取ればネットワーク分離も簡単に行えます。

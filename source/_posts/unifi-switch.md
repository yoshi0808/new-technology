...
1. 予め作成した複数のネットワークを束ねたProfileを作成した上でそれを指定します。
2. 仮想ネットワークバインディングで許可するVLANまたはブロックするVLANを指定します。
...
許可する、ブロックするのはどちらを使っても構いません。個人のように普段使うVLANがおおよそ決まっていて、WANなど特殊なものだけを除外する場合はブロックする方法を選択した方が簡単でしょうか。

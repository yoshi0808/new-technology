...
これはPro-Aggregationの例ですが、Portごとに接続されているクライアントが表示されています。ここで個別のPortを選択すると、そのPortがどのVLANに属するかを設定できます。単一のホストであれば、所属するVLANを指定します。
...
Profile作成時には指定する複数のネットワークのうちどれか1つをネイティブVLANとしてタグ無しに設定します。昨今、デフォルトVLANを変えることがセキュリティ上優位であるとも思えないので、私は管理用VLANは無条件にデフォルトVLAN（VLAN ID=1）をネイティブVLAN（タグ無し）にして各UniFiネットワーク機器は全てデフォルトVLANで管理するようにしています。
...
なお、SFP＋のインタフェースを持つFirewallなどの機器からホームゲートウェイ(HGW)などRJ45を持つ機器に接続するときにはインタフェースの変換が必要となります。スイッチを2Port、LANとは異なるVLANネットワークにし、RJ45 SFP+モジュールを使うことでインタフェースの変換に使うのもVLANの一つの活用方法です。上記の例ではHGWというVLANを作成し、RJ45モジュールをPort26に挿してauひかりから提供されるHGWのRJ45ポートに接続しています。

ネットワークカード（NIC）にSFP＋のRJ45モジュールを使う事はお勧めしません。省電力版のRJ45モジュールを使い短いケーブルであれば、発熱が低く電力が不足することはないかもしれませんが、電力不足によりリンクアップしなかったり2.5Gbpsでリンクアップするなどの事象が発生する場合があります。また、モジュールの発熱によるNICの故障の原因になります。
...
UniFiのL3スイッチはDHCP機能を持ちますが、管理機能やルーティングについてはIPv6には対応していません。スイッチ自身はIPv6アドレスを持てるようですが、現段階でIPv6のL3ルーティングできるようにはなっていません。別の機器でDHCPv6を割り当て、そちらをゲートウェイにするなどが必要です。L2スイッチを含めてIPv6通信が行えないということではありません。

## SFPモジュール

Ubiquitiのモジュール群は廉価なため、光モジュールは純正が間違いないでしょう。OM3ケーブルやDACはさまざまなメーカーのものを使いましたが、UniFi製品で問題が発生したことはありません。RJ45モジュールは特に10GbEのものは低電力を踏まえるとSFPメーカーの製品を選定することになります。

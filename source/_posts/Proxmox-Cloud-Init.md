...
私個人としてはホームユーザーという立場で、データ保管やネットワークなどが中心の運用になっています。NASはSynologyを使い、ネットワークはUniFiを使い、できれば既にあるものはわざわざ自分では作らないという考えであり、過去、自身で構築している主だったものとしてはESXiに乗ったFirewall（Sophos）でした。

Proxmoxになって、クラスター対応ができるようになり、運用の品質が一段上げられると感じました。前述したようにGUIである程度簡単にやれるのはとても便利でしたが、Ubuntuを活用しているとは言い難く、なぜUbuntu Desktopなの？と聞かれればせいぜいネットワークの設定がGUIで簡単、Firewallの設定が簡単だったということくらいでしょうか。
...
大事なことはこれまで人がわかりやすいGUIや簡易な設定（教育コストが低いもの）に重きが置かれていたものが、AIを使って生産性を高める、運用品質を高められるようになったという事です。
...
- `52unattended-upgrades-local` → 自動rebootを **false** に設定（デフォルト）。開発用VMなど任意のタイミングでの再起動が許可される場合は後でtrueに変更する
...
内部でupdateが進んでいるので、最初はrestart requiredが表示されるでしょう。一旦作成してしまえばDiskやCPUを変えながらさまざまなホストを作成できます。
...
Cloud-Initが参照したuser-dataはCD-ROMのようにマウントされたままになっています。検証のためにIPアドレスを変更したり、構成を変更した場合は再度user-dataが読み込まれます。固定IPに変更してそのまま内容を変更しないなど安定的なサーバーに昇格させる場合は、Cloud-initのデバイスをProxomoxのハードウェアから削除するのがおすすめです（再構成が走り例えば追加で設定した/etc/hostsなどが元のuse-dataに置き換えられる等があります）。

### テンプレートの更新

Ubuntu OSは定期的にバージョンアップしていきますが、そんなに頻繁にテンプレートの更新はせずとも、Cloud Init直後に自動更新が入りますから1年に1回程度の見直しなどで良いでしょう。user-dataはVM作成後もCloud-initから呼び出されるため、新しいテンプレートを作成する場合には新しいuser-dataを作成されることをお勧めします。
...
``` bash
qm set 9001 --cicustom "user=Synology-nfs:snippets/user-data-ubuntu-2604.yaml"
```
